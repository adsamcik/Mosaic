...
use mosaic_client::{
    ClientErrorCode, UploadJobEvent, UploadJobPhase, UploadJobSnapshot, UploadShardRef, Uuid,
    advance_upload_job, snapshot_schema,
};
...
fn uploaded_snapshot(phase: UploadJobPhase) -> UploadJobSnapshot {
...
fn crash_replay_vectors_for_ack_drop_boundaries() {
    ...
    let manifest_unknown = advance_upload_job(
        uploaded_snapshot(UploadJobPhase::CreatingManifest),
...
fn native_round_trip_byte_equality_golden_serialization() {
    let bytes = uploaded_snapshot(UploadJobPhase::CreatingManifest).to_canonical_cbor();
...
#[test]
fn replay_effect_ack_idempotent_same_id() {
    let applied = advance_upload_job(
        snapshot(UploadJobPhase::EncryptingShard),
        UploadJobEvent::ShardEncrypted {
            effect_id: uuid(70),
            shard: shard(0),
    ...
    .unwrap()
    .next_snapshot;
    let revision_after_event = applied.snapshot_revision;
    ...
    .unwrap()
    .next_snapshot;
    let revision_after_ack = acknowledged.snapshot_revision;

    let replayed = advance_upload_job(
        acknowledged.clone(),
        UploadJobEvent::EffectAck {
            effect_id: uuid(70),
        },
    ...
    assert_eq!(acknowledged.last_acknowledged_effect_id, Some(uuid(70)));
    assert_eq!(acknowledged.last_applied_event_id, Some(uuid(70)));
    assert_eq!(revision_after_ack, revision_after_event + 1);
    assert_eq!(replayed.next_snapshot, acknowledged);
    assert!(replayed.effects.is_empty());
}

#[test]
fn effect_ack_with_older_id_is_rejected() {
    let acknowledged = advance_upload_job(
        snapshot(UploadJobPhase::EncryptingShard),
        UploadJobEvent::EffectAck {
            effect_id: uuid(90),
...
#[test]
fn effect_ack_does_not_clobber_newer_watermark() {
    let acknowledged = advance_upload_job(
        snapshot(UploadJobPhase::EncryptingShard),
        UploadJobEvent::EffectAck {
            effect_id: uuid(90),
    ...
    assert_eq!(error.code, ClientErrorCode::ClientCoreInvalidTransition);
    assert_eq!(acknowledged.last_acknowledged_effect_id, Some(uuid(90)));
}

#[test]
fn advance_rejects_nil_uuid_event_id() {
    let error = advance_upload_job(
        snapshot(UploadJobPhase::EncryptingShard),
        UploadJobEvent::ShardEncrypted {
            effect_id: Uuid::from_bytes([0; 16]),
            shard: shard(0),
        },
    )
    .unwrap_err();
...
#[test]
fn advance_rejects_non_v7_uuid() {
    let mut bytes = [0x44; 16];
    bytes[6] = 0x40;
    bytes[8] = 0x80;
    let error = advance_upload_job(
        snapshot(UploadJobPhase::EncryptingShard),
        UploadJobEvent::ShardEncrypted {
            effect_id: Uuid::from_bytes(bytes),
            shard: shard(0),
        },
    )
    .unwrap_err();
