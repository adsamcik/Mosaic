...
fn derive_session_salt_v2_is_deterministic() {
    let a = derive_session_salt_v2(DOMAIN, USERNAME)
        .unwrap_or_else(|e| panic!("v2 derive failed: {e:?}"));
    let b = derive_session_salt_v2(DOMAIN, USERNAME)
        .unwrap_or_else(|e| panic!("v2 derive failed: {e:?}"));
    assert_eq!(a, b);
}
...
fn derive_session_salt_v2_distinguishes_boundary_shifts() {
    ...
    let v1_left = derive_session_salt("alice.example.com", "bob")
        .unwrap_or_else(|e| panic!("v1 derive failed: {e:?}"));
    let v1_right = derive_session_salt("alice.example.comb", "ob")
        .unwrap_or_else(|e| panic!("v1 derive failed: {e:?}"));
    assert_eq!(
        v1_left, v1_right,
    ...
    let v2_left = derive_session_salt_v2("alice.example.com", "bob")
        .unwrap_or_else(|e| panic!("v2 derive failed: {e:?}"));
    let v2_right = derive_session_salt_v2("alice.example.comb", "ob")
        .unwrap_or_else(|e| panic!("v2 derive failed: {e:?}"));
    assert_ne!(
        v2_left, v2_right,
...
fn derive_session_salt_v2_differs_from_v1_for_same_input() {
    ...
    let v1 =
        derive_session_salt(DOMAIN, USERNAME).unwrap_or_else(|e| panic!("v1 derive failed: {e:?}"));
    let v2 = derive_session_salt_v2(DOMAIN, USERNAME)
        .unwrap_or_else(|e| panic!("v2 derive failed: {e:?}"));
    assert_ne!(v1, v2);
}
...
fn derive_session_salt_v2_accepts_empty_username() {
    let salt = derive_session_salt_v2(DOMAIN, "")
        .unwrap_or_else(|e| panic!("v2 empty username failed: {e:?}"));
    assert_eq!(salt.len(), 16);
    ...
    let v1 = derive_session_salt(DOMAIN, "")
        .unwrap_or_else(|e| panic!("v1 empty username failed: {e:?}"));
    assert_ne!(salt, v1);
}
