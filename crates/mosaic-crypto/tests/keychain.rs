use mosaic_crypto::{
    ...
    derive_auth_signing_keypair_from_l0, derive_l0_master, derive_l1_root, unwrap_account_key,
...
fn login_l0_reuse_is_faster_than_two_argon2_passes() {
    ...
    let _old_auth = match derive_auth_signing_keypair_from_l0(&old_l0_for_auth, &USER_SALT) {
        Ok(value) => value,
    ...
    let _new_auth = match derive_auth_signing_keypair_from_l0(&l0_master, &USER_SALT) {
        Ok(value) => value,
    ...
    assert!(
        new_duration.as_nanos().saturating_mul(2) < old_duration.as_nanos(),
        "expected one Argon2 + two HKDFs to take <50% of two Argon2 + two HKDFs; old={old_duration:?}, new={new_duration:?}"
    );
}
