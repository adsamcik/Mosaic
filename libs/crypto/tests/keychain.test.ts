...
describe('keychain', () => {
  ...
  it('rewraps account key with new password', async () => {
    const keys = await deriveKeys(password, userSalt, accountSalt, fastParams);
    const newWrapped = await rewrapAccountKey(
      keys.accountKey,
      'new-password',
      userSalt,
      accountSalt,
  ...
  it('rejects invalid salt lengths', async () => {
    await expect(
      deriveKeys(password, new Uint8Array(8), accountSalt, fastParams),
    ).rejects.toThrow();
    await expect(
      deriveKeys(password, userSalt, new Uint8Array(8), fastParams),
    ).rejects.toThrow();
  });
  ...
  it('rejects wrapped account key too short', async () => {
    ...
    await expect(
      unwrapAccountKey(password, userSalt, accountSalt, tooShort, fastParams),
  ...
  describe('context string determinism', () => {
    it('produces identical rootKey for identical inputs across multiple calls', async () => {
      ...
      const keys3 = await deriveKeysInternal(
        fixedPassword,
        fixedUserSalt,
        fixedAccountSalt,
        fastParams,
      ...
      const rootKeyHex = Array.from(keys1.rootKey)
    ...
    it('rootKey snapshot matches expected value (kills ROOT_KEY_CONTEXT mutant)', async () => {
      ...
      const rootKeyHex = Array.from(keys.rootKey)
        .map((b: number) => b.toString(16).padStart(2, '0'))
        .join('');
      ...
      expect(rootKeyHex).toBe(
        'd79b27da4128b04b310fbef3265990982aa8d44d17b12bebfc829f9afff2dc17',
      );
    });
