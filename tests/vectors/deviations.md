...
|----|--------|--------------|----------------|--------------|
| `tier-key-wrap` | `tests/vectors/tier_key_wrap.json` | `crypto_secretbox` (XSalsa20-Poly1305) over `nonce \|\| ciphertext` | `XChaCha20-Poly1305` (AEAD trait) over `nonce \|\| ciphertext` | Decide which AEAD wins for v1; align both ends. Most likely path: keep Rust link-tier wraps on the AAD-bound XChaCha20-Poly1305 path (`mosaic:l3-link-tier-key:v1`) because Mosaic has no deployed share-link records to migrate. |
