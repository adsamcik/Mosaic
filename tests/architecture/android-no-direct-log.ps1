...
$Patterns = @(
  ...
  @{ Name = 'System.out/err.println';       Regex = '\bSystem\s*\.\s*(out|err)\s*\.\s*println\b' },
  @{ Name = 'LoggerFactory.getLogger';      Regex = '\bLoggerFactory\s*\.\s*getLogger\b' },
  @{ Name = 'java.util.logging.Logger';     Regex = '\bjava\s*\.\s*util\s*\.\s*logging\s*\.\s*Logger\b' },
