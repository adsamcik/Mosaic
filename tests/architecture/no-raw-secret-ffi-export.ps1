...
$allowlist = @{
  ...
  'crates/mosaic-wasm/src/lib.rs::unwrap_session_cache_blob' = 'SAFE: Decrypts only session-cache AES-GCM blobs via an open Rust handle; the wrap key never leaves Rust.'
  'crates/mosaic-wasm/src/lib.rs::wrap_session_cache_blob_js' = 'SAFE: JS-visible session-cache wrap; same handle-only contract as the Rust wrapper.'
  'crates/mosaic-wasm/src/lib.rs::unwrap_session_cache_blob_js' = 'SAFE: JS-visible session-cache unwrap; same handle-only contract as the Rust wrapper.'
  'crates/mosaic-wasm/src/lib.rs::wrap_link_tier_blob' = 'SAFE: Returns AES-GCM ciphertext bound to ephemeral link-tier wrap handle; the wrap key never crosses the FFI boundary. Replaces in-JS crypto.subtle generateKey+encrypt path on the frontend link-tier key store (Wave 4B).'
  ...
  'crates/mosaic-wasm/src/lib.rs::wrap_link_tier_blob_js' = 'SAFE: JS-visible link-tier wrap; same handle-only contract as the Rust wrapper.'
  'crates/mosaic-wasm/src/lib.rs::unwrap_link_tier_blob_js' = 'SAFE: JS-visible link-tier unwrap; same handle-only contract as the Rust wrapper.'
