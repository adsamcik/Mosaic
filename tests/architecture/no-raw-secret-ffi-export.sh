...
allowlist = {
    ...
    # Wave 4A session-cache wrap/unwrap: AES-GCM ciphertext keyed by an ephemeral Rust handle id; the AES wrap key never leaves Rust.
    "crates/mosaic-wasm/src/lib.rs::wrap_session_cache_blob": "SAFE: Returns AES-GCM ciphertext bound to ephemeral session-cache wrap handle; the wrap key never crosses the FFI boundary. Replaces in-JS crypto.subtle generateKey+encrypt path on the frontend (Wave 4B). ADR-006/ADR-021 handle-only contract preserved.",
    "crates/mosaic-wasm/src/lib.rs::unwrap_session_cache_blob": "SAFE: Decrypts only session-cache AES-GCM blobs via an open Rust handle; the wrap key never leaves Rust. Input is ciphertext, output is the caller-provided original plaintext.",
    "crates/mosaic-wasm/src/lib.rs::wrap_session_cache_blob_js": "SAFE: JS-visible session-cache wrap; same handle-only contract as the Rust wrapper above.",
    "crates/mosaic-wasm/src/lib.rs::unwrap_session_cache_blob_js": "SAFE: JS-visible session-cache unwrap; same handle-only contract as the Rust wrapper above.",
    # Wave 4A link-tier wrap/unwrap: AES-GCM ciphertext keyed by an ephemeral link-tier wrap handle; the AES wrap key never leaves Rust.
    "crates/mosaic-wasm/src/lib.rs::wrap_link_tier_blob": "SAFE: Returns AES-GCM ciphertext bound to ephemeral link-tier wrap handle; the wrap key never crosses the FFI boundary. Replaces in-JS crypto.subtle generateKey+encrypt path on the frontend link-tier key store (Wave 4B). ADR-006/ADR-021 handle-only contract preserved.",
    "crates/mosaic-wasm/src/lib.rs::unwrap_link_tier_blob": "SAFE: Decrypts only link-tier AES-GCM blobs via an open Rust handle; the wrap key never leaves Rust.",
    "crates/mosaic-wasm/src/lib.rs::wrap_link_tier_blob_js": "SAFE: JS-visible link-tier wrap; same handle-only contract as the Rust wrapper above.",
    "crates/mosaic-wasm/src/lib.rs::unwrap_link_tier_blob_js": "SAFE: JS-visible link-tier unwrap; same handle-only contract as the Rust wrapper above.",
    "crates/mosaic-uniffi/src/lib.rs::wrap_session_cache_blob": "SAFE: Returns AES-GCM ciphertext bound to ephemeral session-cache wrap handle; the wrap key never crosses the UniFFI boundary.",
    "crates/mosaic-uniffi/src/lib.rs::unwrap_session_cache_blob": "SAFE: Decrypts only session-cache AES-GCM blobs via an open Rust handle; the wrap key never leaves Rust.",
    "crates/mosaic-uniffi/src/lib.rs::wrap_link_tier_blob": "SAFE: Returns AES-GCM ciphertext bound to ephemeral link-tier wrap handle; the wrap key never crosses the UniFFI boundary.",
    "crates/mosaic-uniffi/src/lib.rs::unwrap_link_tier_blob": "SAFE: Decrypts only link-tier AES-GCM blobs via an open Rust handle; the wrap key never leaves Rust.",
    # Returns ACCOUNT_DATA_AAD AEAD ciphertext; L2 account key remains inside Rust handle registry.
    "crates/mosaic-uniffi/src/lib.rs::wrap_with_account_handle": "SAFE: Returns ACCOUNT_DATA_AAD AEAD ciphertext; L2 account key remains inside Rust handle registry.",
    # Decrypts only ACCOUNT_DATA_AAD blobs via an open handle; does not expose L2 or seed-domain key material.
    "crates/mosaic-uniffi/src/lib.rs::unwrap_with_account_handle": "SAFE: Decrypts only ACCOUNT_DATA_AAD blobs via an open handle; does not expose L2 or seed-domain key material.",
    ...
    # Returns NEW L2 ciphertext wrapped under the NEW password-derived L1; L2 plaintext never leaves Rust handle. Used for password rotation (v1.0.x auth-03).
    "crates/mosaic-wasm/src/lib.rs::rewrap_account_key_with_handle": "SAFE: Returns NEW wrapped account-key ciphertext (L2 re-encrypted under L1_new derived from caller-provided newPassword + newUserSalt). L2 plaintext stays inside the Rust handle registry. The newPassword input is zeroized on return per the explicit Drop impl. Replaces in-JS rewrap path that would have required L2 to cross FFI. Used by password rotation flow (apps/web/src/lib/password-rotation.ts).",
    "crates/mosaic-wasm/src/lib.rs::rewrap_account_key_with_handle_js": "SAFE: JS-visible password-rotation rewrap; same handle-only contract as the Rust wrapper above. L2 plaintext stays inside the Rust handle registry; newPassword zeroized on return.",
    ...
    # Returns deterministic 16-byte LocalAuth account-salt KDF parameter derived from user salt; output contains no key material or bearer authority.
    "crates/mosaic-uniffi/src/lib.rs::derive_account_salt": "SAFE: Returns deterministic 16-byte LocalAuth account-salt KDF parameter derived from user salt; output contains no key material or bearer authority.",
}
