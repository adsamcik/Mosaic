...
public class SecurityTests
{
    ...
    // Create-path Forbid tests (NonMember / Viewer / RevokedMember) live in
    // Integration/ManifestsAuthorizationTests.cs because they require a real
    // PostgreSQL instance to exercise the SELECT ... FOR UPDATE row-locking
    // code path in ManifestsController.Create. (v1.0.1 s26.)
    ...
    public async Task Security_Manifests_NonMemberCannotGetManifest()
    {
        ...
        var controller = new ManifestsController(db, new MockQuotaSettingsService(), new MockCurrentUserService(db), NullLoggerFactory.CreateNullLogger<ManifestsController>())
        {
            ControllerContext = new ControllerContext
            {
                HttpContext = TestHttpContext.Create(UserB)
