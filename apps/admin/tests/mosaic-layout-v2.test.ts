...
function createPhoto(
  overrides: Partial<PhotoMeta> & { id: string },
): PhotoMeta {
  return {
...
function createPortraitPhoto(
  id: string,
  aspectRatio: number = 0.67,
): PhotoMeta {
  return createPhoto({
    id,
    width: Math.round(1200 * aspectRatio),
...
function createPhotoWithDescription(
  id: string,
  description: string,
): PhotoMeta {
  return createPhoto({
  ...
  describe('Basic Justified Layout', () => {
    ...
    it('should fill container width exactly for complete rows', () => {
      ...
      const totalWidth = items.reduce((sum, item, i) => {
        return (
          sum + item.rect.width + (i < items.length - 1 ? defaultConfig.gap : 0)
        );
      }, 0);

      // Should fill container width (allowing for rounding)
      expect(totalWidth).toBeGreaterThanOrEqual(
        defaultConfig.containerWidth - 5,
      );
...
describe('Enhanced Mosaic Layout - Smart Tiles', () => {
  describe('Description Tile Insertion', () => {
    it('should create story tiles for photos with descriptions', () => {
      const photos = [
        createPhotoWithDescription(
          'story-photo',
          'This is a meaningful description that tells a story about this particular photo and provides context.',
        ),
    ...
    it('should skip description tiles when disabled', () => {
      const photos = [
        createPhotoWithDescription(
          'story-photo',
          'This is a long meaningful description.',
        ),
      ];
  ...
  describe('Map Tile Insertion', () => {
    it('should create map cluster tiles for GPS-tagged photos', () => {
      const photos = [
        createPhotoWithGps('gps-1', 40.7128, -74.006), // NYC
        createPhotoWithGps('gps-2', 40.7129, -74.0061),
        createPhotoWithGps('gps-3', 40.713, -74.0062),
      ];
    ...
    it('should not create map tiles with insufficient GPS photos', () => {
      const photos = [
        createPhotoWithGps('gps-1', 40.7128, -74.006),
        createPhotoWithGps('gps-2', 40.7129, -74.0061),
...
describe('Enhanced Mosaic Layout - Edge Cases', () => {
  ...
  it('should handle extremely wide photos', () => {
    ...
    expect(items).toHaveLength(1);
    expect(items[0].rect.width).toBeLessThanOrEqual(
      defaultConfig.containerWidth,
    );
  });
...
describe('Enhanced Mosaic Layout - Integration', () => {
  it('should create a realistic mixed layout with story and map tiles', () => {
    ...
    const photos = [
      createPhotoWithDescription(
        'story-1',
        'This is a beautiful sunset photo taken during our vacation to the beach. The colors were amazing!',
      ),
      createPhotoWithGps('gps-1', 40.7128, -74.006),
      createPhotoWithGps('gps-2', 40.7129, -74.0061),
      createPhotoWithGps('gps-3', 40.713, -74.0062),
    ...
    for (const row of rows) {
      ...
      for (let i = 1; i < sortedItems.length; i++) {
        ...
        expect(prev.rect.left + prev.rect.width).toBeLessThanOrEqual(
          curr.rect.left,
        );
      }
    }
