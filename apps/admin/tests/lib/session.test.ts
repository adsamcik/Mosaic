...
async function getSessionModule() {
  ...
  const albumMetadataService = await import('../../src/lib/album-metadata-service');
...
describe('session', () => {
  ...
  describe('clearSession (logout)', () => {
    ...
    it('clears data caches before crypto keys', async () => {
      ...
      const cacheEncryptionKeyIndex = callOrder.indexOf('clearCacheEncryptionKey');
    ...
    it('continues clearing other caches even if one throws', async () => {
      ...
      await expect(session.logout()).rejects.toThrow('BlurHash cache clear failed');
