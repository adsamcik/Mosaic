...
class SyncCoordinator {
  ...
  private handleSyncTimeout(albumId: string, assetId: string): void {
    ...
    log.warn(`Sync timeout for ${key} - item not confirmed after ${SYNC_TIMEOUT_MS}ms`);
  ...
  private applyDelta(
    ...
    for (const { assetId, photo } of delta.promoted) {
      const promotionData: Pick<PhotoItem, 'encryptedMetadata' | 'thumbnailUrl' | 'createdAt'> = {
...
const SyncCoordinatorContext = createContext<SyncCoordinatorContextValue | null>(null);
...
export function SyncCoordinatorProvider({ children }: SyncCoordinatorProviderProps): ReactNode {
  ...
  const value: SyncCoordinatorContextValue = {
    registerPendingSync: (albumId, assetId) => syncCoordinator.registerPendingSync(albumId, assetId),
    cancelPendingSync: (albumId, assetId) => syncCoordinator.cancelPendingSync(albumId, assetId),
  };
...
export function useSyncCoordinator(): SyncCoordinatorContextValue {
  ...
  if (!context) {
    throw new Error('useSyncCoordinator must be used within a SyncCoordinatorProvider');
  }
