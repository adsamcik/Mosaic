...
export function PendingPhotoThumbnail({ task }: PendingPhotoThumbnailProps) {
  ...
  const displayProgress = useMemo(() => {
    ...
    return 0;
  }, [task.status, task.currentAction, isEncrypting, isUploading, isFinalizing, task.progress]);

  const showProgress = task.status !== 'queued' && task.currentAction !== 'pending' && !task.error;

  const statusText = useMemo(() => {
    if (task.error) return 'Error';
    if (task.status === 'queued' || task.currentAction === 'pending') return 'Queued';
    if (isEncrypting) return 'Encrypting...';
    if (isUploading) return 'Uploading...';
    if (isFinalizing) return 'Finalizing...';
    return 'Waiting...';
  }, [task.status, task.currentAction, task.error, isEncrypting, isUploading, isFinalizing]);
