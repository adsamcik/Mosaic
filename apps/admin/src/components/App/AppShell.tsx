        ...
        <header className="app-header">
          <div className="header-left">
            ...
            {currentView === 'gallery' && selectedAlbumId && (
              <button onClick={handleBackToAlbums} className="back-button" title={t('navigation.backToAlbums')}>
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
                <span>{t('navigation.albums')}</span>
              </button>
            )}
            {currentView === 'settings' && (
              <button onClick={handleBackFromSettings} className="back-button" title={t('common.back')}>
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
                <span>{t('common.back')}</span>
              </button>
            )}
            {currentView === 'admin' && (
              <button onClick={handleBackFromAdmin} className="back-button" title={t('common.back')}>
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
              <span>{t('common.back')}</span>
            </button>
          )}
