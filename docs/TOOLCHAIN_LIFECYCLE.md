...
> **Status:** Lifecycle-policy snapshot; the example pins below are historical
> and do not identify a shipped v1.0.2 release. Exact current pins live in the
> manifests and lockfiles. See [RELEASE_STATE.md](RELEASE_STATE.md).
...
| Toolchain | Historical example pin | Upstream cadence | Our floor policy | Our ceiling policy |
|---|---|---|---|---|
| **Rust (stable)** | `1.83.x` (historical planning snapshot) | 6 weeks per release; 1-year LTS-equivalent through Ferrocene | Floor = stable - 2 (≈12 weeks behind tip) so a single CI runner update can keep up | Ceiling = current stable; no MSRV "back-compat" maze for ≤50-user audience |
