...
6. **AEAD domain-separation labels.** Account-key wraps use frozen AAD labels:
   `mosaic:l3-epoch-seed:v1`, `mosaic:l3-identity-seed:v1`, and
   `mosaic:account-wrapped-data:v1`. Cross-domain unwrap must fail
   authentication and return no plaintext bytes.
