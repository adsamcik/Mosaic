...
The server sees the coarse `assetType` enum value in the manifest JSON request body (per ADR-022) for tier-validation and storage purposes, but does not see plaintext media or detailed metadata. Asset-type *compatibility* validation remains client-side except for enum-membership validation: the backend may reject values outside `Image | Video | LiveImage`, but it does not inspect encrypted metadata or sidecar bytes to prove that the claimed enum matches the media. The leakage budget for v1 includes the asset-class enum (`Image` / `Video` / `LiveImage`) as documented in ADR-022. A malicious manifest claiming `assetType = Image` while supplying tier 2 = poster-as-image and tier 3 = video bytes creates an upload-side decryption mismatch but no plaintext server-side anomaly; the receiving client (web gallery, share-link viewer) rejects with `VideoTierShapeRejected` when sidecar discriminator and `assetType` disagree. Q-final-1's negative-fixture test exercises this case explicitly.
